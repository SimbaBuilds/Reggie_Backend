...
# GET STUDENT NAME VIA EMAIL CORRESPONDENCE WITH REGISTRAR
def get_student_name(csv_path, pdf_path, creds, gmail_service, message, label_name):


    listified_student = get_name_from_registrar(pdf_path, creds, gmail_service, message, label_name) #email corrspondence with registrar 
...
def process_hundred_messages(creds, gmail_service, drive_service, email_label_name, root_drive_folder_name, csv_path):
    ...
    for i in range(len(messages)):
        ...
        for pdf_path in pdf_paths:
            ...
            best_match, score, item = get_student_name(csv_path, pdf_path, creds, gmail_service, messages[i], email_label_name) #email corrspondence with registrar
